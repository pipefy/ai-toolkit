...
- **`update_card` / `fill_card_phase_fields` return `CARD_UPDATE_PARTIALLY_APPLIED`:** part of the batch was written and part was rejected. Do not resend the whole `field_updates` list: on `operation` ADD that appends the applied values twice. Retry only the entries in `rejected_fields`, each of which carries the API's own message. `applied_field_ids` is confirmed by a re-read of the card; when `verified` is `false` that read did not run, so retry only the rejected fields and do not resend an ADD for any other requested id.
